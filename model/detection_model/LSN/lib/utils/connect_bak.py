...
def connect(image,pred_mask,bbox,threshold = 0.5):
...
def generate_proposal(image,pred_mask,bbox):
    ...
    for idx,mask in enumerate(proposal_mask):
        ...
        #     cv2.circle(subimage,tuple(centnewpts[i]),1,(0,255,0),1)
        ...
        vis = np.zeros((len(centcentpts),1))
        print(vis)
        vis[0]=1
        newcentcentpts = []
        print(len(centcentpts))
        newcentcentpts.append(centcentpts[0])
        for i in range(len(centcentpts)-1):
            minlen = 1000000000
            minidx = -1
            for j in range(len(centcentpts)):
                if vis[j]==1:
                    continue
                dis = distance_point(centcentpts[j],newcentcentpts[len(newcentcentpts)-1])
                if dis<minlen:
                    minlen = dis
                    minidx=j
            vis[minidx]=1
            newcentcentpts.append(centcentpts[minidx])
