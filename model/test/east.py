...
def test_east(config_file):


    import sys
    sys.path.append('./detection_model/EAST')
    sys.path.append('/home/cjy/FudanOCR/detection_model/EAST/calculate')
    ...
    from calculate.Pascal_VOC import output
    ...
    # TODO 这里需要一些适配处理
    FLAGS = read_config_file(config_file)
    ...
    def get_images_icdar2015():
        ...
        image_names = [os.path.join(FLAGS.test_data_path, image_name) for image_name in image_names if
                       image_name[0] != '.']
        if DEBUG:
    ...
    def detect(image, score_map, geo_map, timer, im_fn, score_map_thresh=0.8, box_thresh=0.3, nms_thres=0.1):
        ...
        text_box_restored1 = restore_rectangle(xy_text1[:, ::-1] * 4,
                                               geo_map1[xy_text1[:, 0], xy_text1[:, 1], :])  # N*4*2
        text_box_restored2 = restore_rectangle(xy_text2[:, ::-1] * 8,
                                               geo_map2[xy_text2[:, 0], xy_text2[:, 1], :])  # N*4*2
    ...
    def rescore(im, boxes, score_map):
        ...
        area_intersect = [math.floor(1 / (1 + np.exp(-50 * (item - area_intersect_mean))) * 1000) / 1000 for item in
                          area_intersect]
    ...
    with tf.get_default_graph().as_default():
        ...
        with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as sess:
            ...
            for im_fn in im_fn_list:
                ...
                im_resized, (ratio_h, ratio_w) = resize_image(im,
                                                              max_side_len=1280)  # 将图片成比例放缩，最大边不超过max_side_len，并且是32的倍数
                ...
                if DEBUG:
                    ...
                    if not FLAGS.no_write_images:
                        ...
                        score_path = os.path.join(FLAGS.output_dir,
                                                  '%s_score.jpg' % os.path.basename(im_fn).split('.')[0])
        ...
        # if FLAGS.write_json:
            ...
            import os
            path = os.path.join(FLAGS.output_dir, 'txt')
            f = os.listdir(path)
            ...
            with open(FLAGS.json_output,
                      "w") as f:
                f.write(j_output)

    # 得到输出指标
    output(FLAGS.json_output,FLAGS.json_output_gt)
