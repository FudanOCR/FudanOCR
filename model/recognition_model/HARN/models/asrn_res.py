...
class Residual_block(nn.Module):  # 未用到
...
def conv3x3(in_planes, out_planes, stride=(1, 1)):  # 未用到
    return nn.Conv2d(in_planes, out_planes, kernel_size=3,
                     stride=stride, padding=1, bias=False)


def conv1x1(in_planes, out_planes, stride=(1, 1)):  # 未用到
    return nn.Conv2d(in_planes, out_planes, kernel_size=1,
                     stride=stride, padding=0, bias=False)


class GRCL(nn.Module):  # 未用到
...
class ASRN(nn.Module):
    ...
    def __init__(self, imgH, nc, nclass, nh, BidirDecoder=False, CUDA=True):
        ...
        #
        # self.ResNet18 = convnet.ResNet18()
        # self.ResNet34 = convnet.ResNet34()
        # self.ResNet50 = convnet.ResNet50()
        ###---------------------------------------------###


        self.rnn = nn.Sequential(
            BidirectionalLSTM(512, nh, nh),  #  secnn
            # BidirectionalLSTM(2048, nh, nh),
            BidirectionalLSTM(nh, nh, nh),
        )#################
        # # 这里根据不同的网络需要将LSTM输出改为2048或1024
    ...
    def forward(self, input, length, text, text_rev, test=False):
        # conv features
        #conv = self.cnn(input) ################  # #此处要和上面对应
        conv = self.secnn(input)
        # conv = self.se_resnet50(input)
        # conv = self.DenseNet169(input)
