...
class RBoxList(object):
    ...
    def __init__(self, bbox, image_size, mode="xywha"):
        ...
        if mode == "xywha" and bbox.size(-1) != 5:
            raise ValueError(
                "last dimenion of bbox should have a "
                "size of 5, got {}".format(bbox.size(-1))
            )
        elif mode == "xyxyxyxy" and bbox.size(-1) != 8:
            raise ValueError(
                "last dimenion of bbox should have a "
                "size of 8, got {}".format(bbox.size(-1))
            )
        if mode not in ("xywha", "xyxyxyxy"):
            raise ValueError("mode should be 'xywha' or 'xyxyxyxy'")
    ...
    def convert(self, mode):
        if mode not in ("xyxy", "xywha"):
            raise ValueError("mode should be 'xyxy' or 'xywha'")
        if mode == self.mode:
            return self
    ...
    def _split_into_xyxy(self):
        if self.mode == "xyxy":
            ...
            xc_list, yc_list, w_list, h_list, a_list = self.bbox.split(1, dim=-1)
            ...
            for i in range(len(xc_list)):
                xc = xc_list[i].numpy()[0]
                yc = yc_list[i].numpy()[0]
                w = w_list[i].numpy()[0]
                h = h_list[i].numpy()[0]
                a = a_list[i].numpy()[0]

                rotate_bbox = xywha_to_xyxy((xc, yc, w, h, a))

                x_min.append([max(int(np.min(rotate_bbox[:, 0])), 0)])
                y_min.append([max(int(np.min(rotate_bbox[:, 1])), 0)])
                x_max.append([min(int(np.max(rotate_bbox[:, 0])) + 1, image_width-1)])
                y_max.append([min(int(np.max(rotate_bbox[:, 1])) + 1, image_height-1)])
