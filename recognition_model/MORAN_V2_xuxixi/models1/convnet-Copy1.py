...
class Gnet(nn.Module):
    ...
    def forward(self, input):
        #input=input.resize(32,3,128,128)
        print(input.size())
        ghnet = self.model(Variable(input)).cuda()
        ghnet=ghnet.reshape(input.size()[0],512,1,71)
        print(ghnet.size())
        return ghnet
...
class DefaultCNN(nn.Module):
    ...
    def forward(self, input):
        #print(input.size()) #torch.Size([32, 3, 32, 280])#32是bach-size
        #print(type(input)) #<class 'torch.Tensor'>
        conv = self.cnn(input)
        #print(conv.size()) torch.Size([32, 512, 1, 71])
        #print(type(conv)) <class 'torch.Tensor'>
        return conv
