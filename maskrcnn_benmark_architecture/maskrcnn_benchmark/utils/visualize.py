...
def vis_image(img, boxes, cls_prob=None, mode=0, font_file='./fonts/ARIAL.TTF'):
    ...
    for idx in range(len(boxes)):
        ...
        if mode == 1:
            draw.line((int(rotated_pts[0, 0]), int(rotated_pts[0, 1]), int(rotated_pts[1, 0]), int(rotated_pts[1, 1])), fill=(0, 255, 0), width=3)
...
def vis_image_with_words(img, boxes, word_labels, coder, voc_list, cls_prob=None, mode=1, font_file='./fonts/STXINGKA.TTF'):
    # img = cv2.imread(image_path)
    # cv2.setMouseCallback("image", trigger)
    font = ImageFont.truetype(font_file, 32)
    draw = ImageDraw.Draw(img)
    ...
    for idx in range(len(boxes)):
        ...
        if not word_labels is None:
            ...
            draw.rectangle(((x-5, y-30), (x + 32 * len(word_label) + 3, y+10)), fill='yellow', outline=None)
            # cv2.rectangle(img, (x-5, y-30), (x + 20 * len(word_label) + 3, y+10), (0, 255, 255), thickness=-1)
            draw.text((x, y-25), word_label, fill=(33, 33, 33, 128), font=font) #(255, 0, 255)
