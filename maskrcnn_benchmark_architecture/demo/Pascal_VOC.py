...
def eval_func(input_json_path, gt_json_path, iou_threshold = 0.5):
    ...
    for input_img_key, input_cnts in input_dict.items():
        ...
        for gt_id, gt in enumerate(groundtruths):
            ...
            if len(detections) > 0:
                ...
                det_res.append([{
                    'gt':np.squeeze(gt['points']).tolist(),
                    'matched_dets':np.squeeze(detections[best_matched_det_id]['points']).tolist(),
                    'iou':float(iou_table[gt_id, best_matched_det_id])
                }])
        ...
        eval_dict[input_img_key] = det_res
    ...
    eval_jname = 'eval.json'
    eval_jobj = open(eval_jname, 'w')
    eval_jobj.write(json.dumps(eval_dict))
    eval_jobj.close()
    print('done')
