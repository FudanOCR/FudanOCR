...
def boxlist_iou(boxlist1, boxlist2, GPU_ID=0):
    ...
    '''
    # print('overlaps.device: ', torch.from_numpy(overlaps).device)
    # print('boxlist1.bbox.device: ', boxlist1.bbox.device)
    # print('now device: ', torch.device('cuda'))
...
def cat_boxlist(bboxes):
    ...
    assert all(isinstance(bbox, RBoxList) for bbox in bboxes), '{}, {}'.format(
        type(bboxes[0]), type(bboxes[1])
    )
