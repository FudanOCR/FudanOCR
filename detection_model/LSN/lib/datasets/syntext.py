...
# sys.path.insert(0,'./')
# import config
from lib.datasets.proposal_generate_syntext import ProposalGenerate
import math
...
class SynthtextDataset(Dataset):
    def __init__(self, datasetroot,transforms,strides,istraining = True,data='synthtext'):
        ...
        self.samplelist = []
        if data == 'synthtext':
            for foldername in os.listdir(os.path.join(datasetroot,'gt')):
                for gtname in os.listdir(os.path.join(datasetroot,'gt',foldername)):
                    self.samplelist.append([os.path.join(datasetroot,'image',foldername,gtname.split('.')[0]+'.jpg'), os.path.join(datasetroot,'gt',foldername,gtname)])
        elif data == 'icdar':
            for gtname in os.listdir(os.path.join(datasetroot,'gt')):
                self.samplelist.append([os.path.join(datasetroot,'image',gtname.split('.')[0][3:]+'.jpg'), os.path.join(datasetroot,'gt',gtname)])
        elif data == 'totaltext':
            for gtname in os.listdir(os.path.join(datasetroot,'gt')):
...
if __name__ == '__main__':
    datasetroot = config.datasetroot#'/home/shf/fudan_ocr_system/datasets/ICDAR15/Text_Localization'
    # savepath = '/data/2019AAAI/disp/'
    strides = [8,16,32,64]
    dataTransform = transforms.Compose([ToTensor(strides)])
    CtwDataset = SynthtextDataset(datasetroot, dataTransform,strides,istraining=False,data='totaltext')
