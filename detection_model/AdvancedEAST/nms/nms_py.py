# coding=utf-8
# NMS by https://github.com/huoyijie/AdvancedEAST
...
def region_neighbor(region_set):
    region_pixels = np.asarray(list(region_set))
...
def rec_region_merge(region_list, m, S):
    ...
    tmp = []
    for n in S:
        if not region_neighbor(region_list[m]).isdisjoint(region_list[n]) or not region_neighbor(region_list[n]).isdisjoint(region_list[m]):
            # 第m与n相交
            tmp.append(n)
    for d in tmp:
        S.remove(d)
