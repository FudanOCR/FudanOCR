...
import model.detection_model.PixelLink.config as config
import model.detection_model.PixelLink.ImgLib.ImgTransform as ImgTransform
import model.detection_model.PixelLink.ImgLib.util as util
...
class ICDAR15Dataset(Dataset):
    def __init__(self, images_dir, labels_dir):
        ...
        self.all_labels = self.read_labels(labels_dir, 500)
        # elf.all_labels = self.read_labels(labels_dir, config.all_trains)
...
class PixelLinkIC15Dataset(ICDAR15Dataset):
    ...
    def label_to_mask_and_pixel_pos_weight2(label, img_size, version="2s", neighbors=8):
        ...
        for bbox_index, bbox_mask in enumerate(bbox_masks):
            ...
            bbox_contours = util.find_contours(bbox_positive_pixel_mask)
