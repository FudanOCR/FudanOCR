...
from model.detection_model.LSN.lib.model.utils.config import cfg
from model.detection_model.LSN.lib.model.roi_crop.functions.roi_crop import RoICropFunction
import cv2
import pdb
import random
debug = True
...
def _smooth_l1_loss(bbox_pred, bbox_targets, bbox_inside_weights, bbox_outside_weights, sigma=1.0, dim=[1]):
    ...
    print('xxxxxxxx',loss_box)
    for i in sorted(dim, reverse=True):
    #   print(i)
      loss_box = loss_box.sum(i)
    # print(torch.sum(loss_box))
    loss_box = loss_box.mean()
    # cv2.imshow('image',np.ones((500,500,3)))
    # cv2.waitKey(0)
    return loss_box

def _smooth_l1_loss_zz(bbox_pred, bbox_targets, bbox_inside_weights, bbox_outside_weights, sigma=1.0, dim=[1]):
    # print('_smooth_l1_loss\n')
    # print('bbox_pred',bbox_pred)
    # print('bbox_targets',bbox_targets)
    # print('bbox_inside_weights', bbox_inside_weights)
    ...
    #   print(i)
      loss_box = loss_box.sum(i)
    # print(torch.sum(loss_box))
    loss_box = loss_box.sum()/len(cfg.ANCHOR_SCALES)*len(cfg.ANCHOR_RATIOS)*4
    # print(loss_box)
    # cv2.imshow('image',np.ones((500,500,3)))
    # cv2.waitKey(0)
    return loss_box
