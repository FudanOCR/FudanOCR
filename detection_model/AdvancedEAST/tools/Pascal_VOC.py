...
from tools.polygon_wrapper import iou
from tools.polygon_wrapper import iod
    ...
    for input_img_key, input_cnts in tqdm(input_dict.items(), desc='Evaluate'):
        ...
        for gt_id, gt in enumerate(groundtruths):
            ...
            if len(detections) > 0:
                ...
                # identified the best matched detection candidates with current groundtruth
                best_matched_det_id = np.argmax(iou_table[gt_id, :])
        ...
        tqdm.write('{0:12} Precision: {1:.4f}, Recall: {2:.4f}'.format(input_img_key + '.jpg', local_precision, local_recall))
