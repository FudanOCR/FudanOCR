...
def val(dataset, criterion, max_iter=1000): ##########1000
    print('Start val')
    ...
    for i in range(max_iter):
        ...
        print('11111')
        if opt.BidirDecoder:
            print('opt.BidirDecoder',opt.BidirDecoder)
            ...
            for j in range(cpu_images.size(0)):
                ...
                if torch.mean(preds0_prob[text_begin:text_begin+len(sim_preds0[j].split('$')[0]+'$')].cpu()) >\
                 torch.mean(preds1_prob[text_begin:text_begin+len(sim_preds1[j].split('$')[0]+'$')].cpu()):
...
for epoch in range(opt.niter):
    ...
    while i < len(train_loader):
        #print("main,iter %d" %  len(train_loader))
        print('i',i)
