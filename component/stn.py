...
class SpatialTransformer(nn.Module):
    ...
    def __init__(self, opt, use_dropout=False):
        super(SpatialTransformer, self).__init__()
        # self._h, self._w = spatial_dims
        # self._in_ch = in_channels
        # self._ksize = kernel_size
        self.dropout = use_dropout
        self._h, self._w , self._in_ch = opt.IMAGE.IMG_H, opt.IMAGE.IMG_W, opt.IMAGE.IMG_CHANNEL
        self._ksize = 3


        # localization net
        self.conv1 = nn.Conv2d(self._in_ch, 32, kernel_size=self._ksize, stride=1, padding=1,
        ...
        '''Adjust the param according to the image size'''
        self.fc1 = nn.Linear(32 * 4 * 12, 1024)
